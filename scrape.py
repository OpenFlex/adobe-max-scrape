...
def load_speakers():
  while True:
    try:
      speaker_data = simplejson.load(urllib.urlopen(speakerurl))
      speakers = {}
      ...
      print 'Error loading speaker data, retrying'

def load_sessions():
  while True:
    try:
      session_data = simplejson.load(urllib.urlopen(sessionurl))
      sessions = {}
      ...
      print 'Error loading sessions, retrying'

def load_session(id,sessions,list):
  url = sessionurl + id + ".json"
  while True:
    try:
      session = sessions[id]
...
baseurl = 'https://max.adobe.com/api/events/'+eventid
speakerurl = baseurl + '/speakers.json'
sessionurl = baseurl + '/sessions.json'
...
for line in open(sys.argv[1],'r').readlines():
  ...
  session = load_session(sessionid,sessions,sessionlist)
...
write_agenda(sys.argv[2]+'.html',instances,True)
...
print 'CouchDB-compatible JSON written'
